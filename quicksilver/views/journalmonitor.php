                        ...
                        items :[
                            {
                                ...
                                items :[{
                                        ...
                                        items: [
                                            ...
                                            {boxLabel: 'Memorial', name: 'rbfilter', inputValue: 4},
                                            {boxLabel: 'Koreksi', name: 'rbfilter', inputValue: 5}                
                                        ],id:'jmon_filter_by'
                                        ,listeners:{
                                            change:function( vthis, newValue, oldValue, eOpts ){
//                                                console.log(newValue);
                                                
                                                Ext.getCmp('jmon_filter_value').setDisabled(false);
                                                Ext.getCmp('jmon_filter_value').setValue('');
                                                
                                                if (newValue.rbfilter==4){
                                                    Ext.getCmp('jmon_filter_value').setValue('MEMORIAL');
                                                    Ext.getCmp('jmon_filter_value').setDisabled(true);
                                                
//                                                    console.log(newValue);
                                                };
                                                if (newValue.rbfilter==5){
                                                    Ext.getCmp('jmon_filter_value').setValue('KOREKSI');
                                                      Ext.getCmp('jmon_filter_value').setDisabled(true);
                ...
                items:[
                    {
                        ...
                        tbar:{
                            ...
                            items:[{
                                    ...
                                    handler:function(){
                                        //validasi             
                                        var storejm=Ext.getCmp('jmon_grid').getStore();
                                        delete storejm.getProxy().setExtraParam('query',vquery);
                                        ...
                                        if (!Ext.getCmp('jmon_filter').collapsed){                                    
                                            ...
                                            }else if (Ext.getCmp('jmon_filter_by').getValue().rbfilter==4){
                                                parquery.push({name:'transaksi_cd',value:Ext.getCmp('jmon_filter_value').getValue()});                                        
                                            }else if (Ext.getCmp('jmon_filter_by').getValue().rbfilter==5){
                                                parquery.push({name:'transaksi_cd',value:Ext.getCmp('jmon_filter_value').getValue()});                                        
                                            }
                                        }
                                
                                        //                                 console.log(parquery.length);
                                        var vquery=Ext.JSON.encode(parquery);
                                        if (parquery.length>0){
                                              
                                              storejm.getProxy().setExtraParam('query',vquery);
                                              storejm.reload();                                          
//                                            journalMonitor_store.getProxy().setExtraParam("query",Ext.JSON.encode(parquery));
//                                            journalMonitor_store.reload()
//                                            journalMonitor_store.removeAll();
//                                            journalMonitor_store.reload({params:{query:Ext.JSON.encode(parquery)}});
                                        }else{
                                            storejm.loadRecords([]);
                                            Ext.getCmp('pagingjmon').onLoad();  
                                        }
                                        //                                
                                    }
                                },{
                                    ...
                                    handler:function(){
                                        ...
                                        if(gridstr.getTotalCount()>0 ){
                                            ...
                                            console.log(vtitle);
                                            var datagrid=Ext.getCmp('jmon_grid');
...
////                delete storegrid.getProxy().extraParams['query'];

...
class entry_jurnal_model extends MY_MODEL {
    ...
    function get_row_approval($search="",$start,$limit) {
        $sql_search="";
        if ($search != "") {
            $sql_search = "(lower(referensi) LIKE '%" . strtolower($search) . "%' or lower(keterangan) LIKE '%" . strtolower($search) . "%'  or lower(jurnal_type) LIKE '%" . strtolower($search) . "%')";
        }
        $select="nomor_entry,tgl_jurnal,referensi,keterangan,jurnal_type";        

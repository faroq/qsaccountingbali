...
//                    'kelompok'=>$h->kelompok.'-'.$h->nama_kelompok,
    ...
    public function get_max_tb_header($head, $child) {
        $resArr = array();
        $level = 0;
        $jenis = '';
        if (count($head) == 0) {
...
//                    'kelompok'=>$h->kelompok.'-'.$h->nama_kelompok,
    ...
    public function trialbalance_pdf()
    {
        ...
        $this->load->library('report/trialbalance_pdf');
...
//        $pdf->Output();
    ...
    public function trialbalance_header_pdf()
    {
        //$thbl = isset($_GET['thbl']) ? json_decode($this->input->post('thbl', TRUE)) : null;
        $thbl = $_GET['thbl'];
        $head = $this->bm->get_account_level1();
...
//        $pdf->Output();
    ...
    public function gl_pdf() {
        ...
        if ($param[1] == 'on'){
            ...
            $namarekening = $this->bm->callFunction($fungrekening, array($param[5]));
        }
        $this->load->library('report/gl_pdf');
        $pdf=new gl_pdf();
        $pdf->AliasNbPages();
...
//        $pdf->Output();
    ...
    public function gl_pdfA3() {
        ...
        $this->load->library('report/gl_a3');
        $this->load->config('config');
        
//        $pdf = new DaftarSiswaPerkelas('P', 'mm', 'A4');
        $pdf=new gl_a3('L', 'mm', 'A3');
        $pdf->AliasNbPages();
